	...
	for( int itr = 0; itr < nb_iteration; itr++ )    
	{
			...
			for(int i = -STEP; i <= STEP; ++i)
			{
				for(int j = -STEP; j <= STEP; ++j)
				{
						...
						yk+i < m_height)
					{
						...
						double ldist = (m_lvec[(xk+j)*m_height+(yk+i)]-kseedsl[n]);
						double adist = (m_avec[(xk+j)*m_height+(yk+i)]-kseedsa[n]);
						double bdist = (m_bvec[(xk+j)*m_height+(yk+i)]-kseedsb[n]);
						double colordist = ldist*ldist + adist*adist + bdist*bdist;
						double xdist = j;
						double ydist = i;
						double spacedist = xdist*xdist + ydist*ydist;
						double dista = colordist + spacedist*invwt;
						// If distance is lower than the best current one -> association
						if(dista < distvec[(xk+j)*m_height+(yk+i)])
						{
							distvec[(xk+j)*m_height+(yk+i)] = dista;
							klabels[(xk+j)*m_height+(yk+i)] = n;
						}
					}
				}
			}
		...
		for( int r = 0; r < m_width; r++ )   {
			for( int c = 0; c < m_height; c++ )   {
				clustersize[klabels[r*m_height+c]]++;
			 	kseedsx[klabels[r*m_height+c]] += r;
				kseedsy[klabels[r*m_height+c]] += c;
				kseedsa[klabels[r*m_height+c]] += m_avec[r*m_height+c];
				kseedsb[klabels[r*m_height+c]] += m_bvec[r*m_height+c];
				kseedsl[klabels[r*m_height+c]] += m_lvec[r*m_height+c];
			}
		}
		
		// Normalization by the number of pixels in each superpixel
		for( int k = 0; k < numk; k++ )    {
			kseedsx[k] /= clustersize[k];
			kseedsy[k] /= clustersize[k];
			kseedsa[k] /= clustersize[k];
			kseedsb[k] /= clustersize[k];
			kseedsl[k] /= clustersize[k];
		}
	}
